...
var
  ...
  begin
    ...
    begin
      ...
      with rec^[i] do
      begin
        lpszName:=pAnsiChar(StrPCopy(new(TlpszRecipName)^,PAnsiChar(lpRecip^.lpszName)));
        if lpRecip^.lpszAddress=nil then
          lpszAddress:=PAnsiChar(StrPCopy(new(TlpszRecipName)^,lpRecip^.lpszName))
        else
          lpszAddress:=PansiChar(StrPCopy(new(TlpszRecipName)^,lpRecip^.lpszAddress));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        att^[i].lpszPathName:=PAnsiChar(StrPCopy(new(TlpszPathname)^,Attachments[i]));
        att^[i].lpszFileName:=PAnsiChar(StrPCopy(new(TlpszFileName)^,ExtractFileName(Attachments[i])));
      end;
      lpFiles:=@att^;
    end
    else
